...
ShellRoot {
    ...
    Timer {
        ...
        onTriggered: {
            try {
                root.check(String(players.loginSound.source) === String(AudioService.getSoundPath("desktop-login")), "player source resolves the sound file: " + players.loginSound.source);
                root.check(String(players.criticalNotificationSound.source) === String(AudioService.getSoundPath("message-new-instant")), "each player maps its own event");
