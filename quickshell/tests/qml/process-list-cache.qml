...
ShellRoot {
    ...
    readonly property var emptyLabel: view.listView.children.find(c => c.text !== undefined && c.font !== undefined)
    ...
    Timer {
        ...
        onTriggered: {
            try {
                root.check(root.emptyLabel && root.emptyLabel.visible && view.cachedProcesses.length === 0, "first open with no data shows an empty state");

                DgopService.allProcesses = [root.process(1, "init"), root.process(2, "shell")];
                view.active = true;
                input.tryCompare(root.emptyLabel, "visible", false);
                ...
                input.tryCompare(root.emptyLabel, "visible", true);
                root.check(view.cachedProcesses.length === 0 && root.emptyLabel.visible, "a search with no hits shows the empty state");
