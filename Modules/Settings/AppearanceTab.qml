...
Item {
      ...
      Column {
        ...
        Column {
          ...
          DankSlider {
            width: parent.width
            height: 24
            value: Math.round(SettingsData.cornerRadius * 100)
            minimum: 0
            maximum: 100
            unit: ""
            showValue: true
            onSliderValueChanged: (newValue) => {
                            SettingsData.setCornerRadius(newValue / 100);
                        }
          }
        }
      }
    ...
    StyledRect {
      ...
      Column {
        ...
        DankToggle {
          width: parent.width
          text: "GTK Theme Sync"
          description: "Apply theme colors to GTK applications"
          checked: SettingsData.gtkThemeSync
          onToggled: (checked) => {
                        SettingsData.setGtkThemeSync(checked);
                    }
        }

        DankToggle {
          width: parent.width
          text: "Kvantum Theme Sync"
          description: "Apply theme colors to Kvantum applications"
          checked: SettingsData.kvantumThemeSync
          onToggled: (checked) => {
                        SettingsData.setKvantumThemeSync(checked);
                    }
        }
        ...
        DankToggle {
          width: parent.width
          text: "Wallpaper Sync"
          description: "Apply wallpaper to desktop and login screen"
          checked: SettingsData.wallpaperSync
          onToggled: (checked) => {
                        SettingsData.setWallpaperSync(checked);
                    }
        }
      }
    }
  ...
  Process {
    ...
    command: "/usr/bin/killall -USR1 gammastep"
  }
